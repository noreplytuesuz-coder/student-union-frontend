...
function statusBadgeVariant(status: EventStatus): "default" | "primary" | "success" | "warning" | "danger" | "neutral" {
  switch (status) {
    case "upcoming":
      return "primary";
    case "ongoing":
...
export function EventDetails() {
  ...
  const canParticipate = event.status !== "completed" && event.status !== "cancelled" && event.status !== "ongoing";
      ...
      <Card className="p-0 overflow-hidden relative border-none">
        <div className="relative h-64 sm:h-96 w-full">
          ...
          <div className="absolute bottom-6 left-6 right-6">
            ...
            <h1 className="font-display text-4xl sm:text-5xl font-bold text-white mb-2 leading-tight">{event.title}</h1>
          </div>
        </div>
        ...
        <div className="p-6 sm:p-10 bg-[var(--bg-color)] neo-border border-t-0 rounded-b-xl flex flex-col gap-8">
          ...
          <div className="border-t border-gray-200 dark:border-gray-800 pt-8 mt-4">
            ...
            <p className="text-muted-foreground mb-6">
              {t(
                "Direct registration is closed for this event. You can submit a request to join the waitlist, and the organizers will contact you if a spot opens up.",
              )}
            </p>
